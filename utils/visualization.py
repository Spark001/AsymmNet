...
def get_macs(feature_map_size, conv_filter, stride=1, padding=1):
    n = conv_filter[1] * conv_filter[2] * conv_filter[3]  # vector_length
    macs_per_instance = n

    num_instances_per_filter = ((feature_map_size - conv_filter[2] + 2 * padding) // stride) + 1  # for rows
    num_instances_per_filter *= ((feature_map_size - conv_filter[
        3] + 2 * padding) // stride) + 1  # multiplying with cols

    macs_per_filter = num_instances_per_filter * macs_per_instance
    total_macs_per_layer = macs_per_filter * conv_filter[0]  # multiply with number of filters

    acts = num_instances_per_filter * conv_filter[1]
    return total_macs_per_layer, acts
...
def convert_size_num(size_bytes, base=1024):
    return round(1.0 * size_bytes / base / base, 2)


def print_summary_ops(symbol, shape=None, line_length=120, positions=[.40, .52, .60, .68, .73, 1.],
                      quantized_bitwidth=32, verbose_print=True):
    ...
    to_display = ['Layer (type)', 'Output Shape', 'Param #', 'MACs #', 'Prec.', 'Previous Layer']
    ...
    def print_layer_summary(node, out_shape):
        ...
        if op == 'Convolution':
            ...
            kernel_size = int(node['attrs']['kernel'].replace('(', '').replace(')', '').split(',')[0])
            ...
            macs, acts = get_macs(pre_feature_map, conv_filter, stride=stride, padding=pad)
            macs = macs // num_group
            if node["attrs"].get("no_bias", 'False') != 'True':
                cur_param += num_filter

            if kernel_size == 1 and num_group == 1:
                macs_pw = macs
            if num_group > 1:
                macs_dw = macs
            ...
            if node["attrs"].get("no_bias", 'False') == 'True':
                ...
                # add fc
                macs = pre_filter * int(node["attrs"]["num_hidden"])
            else:
                cur_param = (pre_filter + 1) * int(node["attrs"]["num_hidden"])
                macs = (pre_filter + 1) * int(node["attrs"]["num_hidden"])
        ...
        fields = [node['name'] + '(' + op + ')',
                  ...
                  cur_param,
                  '{:.2E}'.format(macs) if macs > 0 else 0,
        ...
        if len(pre_node) > 1:
            for i in range(1, len(pre_node)):
                ...
                if verbose_print:
                    print_row(fields, positions)
        return cur_param, is_quantized, macs, macs_pw, macs_dw, acts
    ...
    for i, node in enumerate(nodes):
        ...
        params, is_quantized, macs, pw_macs, dw_macs, acts = print_layer_summary(nodes[i], out_shape)
        total_params += params
        total_macs += macs
        total_macs_dw += dw_macs
        total_macs_pw += pw_macs
        total_acts += acts
        if is_quantized:
    ...
    total_macs_vanilla = max(total_macs - total_macs_pw - total_macs_dw, 1e-10)
    total_macs_pw_ratio = str(round(total_macs_pw / total_macs, 4) * 100) + '%'
    total_macs_dw_ratio = str(round(total_macs_dw / total_macs, 4) * 100) + '%'
    total_macs_vanilla_ratio = str(round(total_macs_vanilla / total_macs, 4) * 100) + '%'

    if verbose_print:
        print('Total params:     %s' % total_params)
        print('FP params:        %s (%.2f%%)' % (
            total_params - quantized_params, 100 * (total_params - quantized_params) / total_params))
        ...
        print('MACs (total):               %s' % convert_size(total_macs, 1000))
        print('MACs (full-precision):      %s' % convert_size(total_macs - quantized_macs, 1000))
        print('MACs (binary):              %s' % convert_size(quantized_macs, 1000))
        print(
            'MACs (combined):            %s' % convert_size(quantized_macs / 64 + (total_macs - quantized_macs), 1000))
        print('MACs (pointwise):           %s  %s' % ((convert_size(total_macs_pw, 1000)), total_macs_pw_ratio))
        print('MACs (depthwise):           %s  %s' % ((convert_size(total_macs_dw, 1000)), total_macs_dw_ratio))
        print(
            'MACs (vanilla):             %s  %s' % ((convert_size(total_macs_vanilla, 1000)), total_macs_vanilla_ratio))
        print('Acts (convolution):         %s' % (convert_size(total_acts, 1000)))
        print('_' * line_length)

    return convert_size_num(total_params, 1000), convert_size_num(total_macs, 1000), convert_size_num(total_acts, 1000)
